...
from Ambiente_SOMN.make_env2 import make_env
    ...
    for x in range(5):    #### ACMO NUMEROS DE EXECUÇÕES COMPETIDORAS
        run1 = wandb.init(project='Fred_test_SU', #NOME DO PROJETO
                        ...
                        #   name=f"PPO (teste 12, atraso = {atraso:02d}, run: {x + 1:02d})",
                          name=f"PPO (teste 14, experimento {experimento:02d}, run {x + 1:02d})",
        ...
        model.learn(total_timesteps=1_000_000, callback=[checkpoint_callback, wandb_callback])
