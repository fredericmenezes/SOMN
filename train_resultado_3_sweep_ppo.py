...
class RAND(OffPolicyAlgorithm):
    ...
    def learn(self, total_timesteps, callback=None, log_interval=4, tb_log_name="RAND", reset_num_timesteps=True):
        ...
        while self.num_timesteps < total_timesteps:
            # multiplicar por zero para simular agente com a mesma ação
...
def train_and_select_best(alg_class, alg_name, wandb_config, n_evaluations, total_timesteps):
    ...
    for i in range(n_evaluations):
        ...
        model.save(os.path.join(wandb.run.dir, f"{alg_name}_sweep3_model_{wandb.run.name}"))
        wandb.finish()
...
if __name__ == "__main__":
    ...
    with open("./config_sweep_3_ppo.yaml") as file:
    ...
    #                         n_evaluations, 
    #                         total_timesteps)
    # _ = train_and_select_best(DQN, 
    #                         "dqn", 
    #                         config, 
    #                         n_evaluations, 
    #                         total_timesteps)
    _ = train_and_select_best(PPO, 
                            "ppo", 
                            config, 
                            n_evaluations, 
                            total_timesteps)
    # random_mean_reward, num_best_random = train_and_select_best("comp3_act=0_t=200_atraso_real+5_tx_penalidade=10",
    #                                                             RAND,  
    #                                                             "randomico", 
    #                                                             config["randomico"], 
    #                                                             n_evaluations, 
