    ...
    for x in range(5):    #### ACMO NUMEROS DE EXECUÇÕES COMPETIDORAS
        run1 = wandb.init(project='Fred_test_SU', #NOME DO PROJETO
                        ...
                        #   name=f"PPO (teste 12, atraso = {atraso:02d}, run: {x + 1:02d})",
                          name=f"PPO (teste 15, experimento {experimento:02d}, run {x + 1:02d})",
        ...
        model.save(os.path.join(wandb.run.dir, f"Sweep_PPO_Recurrent_param_173 (atraso = {atraso:02d}) run_{x + 1:02d}"))
        wandb.finish()