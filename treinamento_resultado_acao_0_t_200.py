...
def train_and_select_best(alg_class, alg_name, config, n_evaluations, total_timesteps):
    ...
    for i in range(n_evaluations):
        ...
        run = wandb.init(
             ...
             name = f"{alg_name}_comp8_act=0_t=200_run_{i + 1:02d}",
             save_code = True,
             reinit = True
        )
        ...
        model.save(os.path.join("wandb_models", f"{alg_name}_comp8_act=0_t=200_run_{i + 1:02d}"))
        wandb.finish()

    best_model.save(os.path.join("best_model", f"{alg_name}_comp8_act=0_t=200_run_{num:02d}"))
